<template>
  <div class="report-box">
    <div class="box-img" @click="handlerRest">
      <img src="../../assets/img/knowledge/outIcon.png" alt="" />
      ...
      <t-form ref="form" layout="inline">
        ...
        <div class="content-fen">
          <t-form-item label="食物分类:" requiredMark initial-data="123456">
            <t-checkbox-group requiredMark v-model="foodsSet" :options="foodsLabel" @change="onChangeClass" />
          </t-form-item>
        </div>
    ...
    <div class="box-details">
      <div v-if="knowVinList != ''" class="box-details-item" v-for="(item, index) of knowVinList" :key="item.id">
        <t-checkbox :checked="setArrt.newArr.includes(item.id)" @change="onChange(item)" :key="constKey"></t-checkbox>
...
import { knowList, knowDelEditList, knowDict, knowInfoList } from '../../request/modules/knowledge';
import { useRouter, useRoute } from 'vue-router';
const route = useRoute();
const router = useRouter();
const knowVinList = ref<any>([]);
const formName = ref<any>(''); //反显名称
const formImage = ref<any>(''); //图片展示
const fileList = ref<any>([]);
const headers = ref<any>({}); //图片头部携带
const imgFile = ref<any>([]); //图片
const constKey = ref<any>('1');
...
const getKnowLedgeList = () => {
  knowInfoList(setArrt.searchData).then((res: any) => {
    foodsSet.value = res.data.categoryIds;
    console.log(res, 'checkedValue.value ');
    console.log(foodsSet.value, ' foodsSet');

    knowVinList.value = res.data.statusDTOList.data;
    formName.value = res.data.comboName;
    formImage.value = res.data.comboImage;
    fileList.value[0] = {
      name: '',
      url: res.data.comboImage
    };
    setArrt.total = res.data.statusDTOList.total;
    setArrt.newArr = res.data?.statusDTOList.data

    ...
    <div class="basicsForm">
      ...
      <div class="search-box">
        <t-form ref="form" layout="vertical" :data="formSearch" requiredMark :colon="true">
          ...
          <t-form-item v-if="selectStatus == 2" label="优惠额度" class="w50" name="limit">
            <t-input-number style="width: 350px" size="large" :min="0" @change="onOverMax" @keyup="overMax" placeholder="请输入优惠额度" theme="normal" v-model="formSearch.discount">
              <template #suffix><span>元</span></template>
            </t-input-number>
          </t-form-item>
          <div class="discount-box" v-if="selectStatus == 1">
          ...
          <t-form-item label="库存量" class="w50" name="name">
            <t-input size="large" @keyup="formSearch.stock = formSearch.stock.replace(/^(0+)|[^\d]+/g, '')" @change="onInventory" :maxlength="3" :allowInputOverMax="false" suffix="张" placeholder="请输入库存量" v-model="formSearch.stock" />
          </t-form-item>
    ...
    <div class="bindProduct">
      <div class="productCommon">
        ...
        <div class="productPrice" v-if="formSearch.discount && formSearch.price">
          产品优惠后价格:<span class="price">￥{{ formSearch.afterDiscountPrice.toFixed(2) }}</span>
        </div>
      </div>
      <div class="tab-box" style="padding-bottom: 15px">
        <t-table style="padding: 20px" row-key="id" :bordered="false" stripe lazy-load v-model:selected-row-keys="selectedRowKeys" @select-change="rehandleSelectChange" :data="tableData" :columns="(productColumns as PrimaryTableCol<TableRowData>[])" table-layout="auto" size="medium">
...
import { channelProduct, addCoupon } from '../../request/modules/MarketApi';
...
let theOldData = JSON.stringify(formSearch);
//----- 加载列表数据 ---------
onMounted(() => {
  loadTableData();
  getOrganizationList();
...
const rehandleSelectChange = (RowKeys: Array<string | number>, options: SelectOptions<any>) => {
  ...
  if (formSearch.couponType == 1) {
    formSearch.afterDiscountPrice = formSearch.price * (formSearch.discount / 10);
  }
};
...
const verify = () => {
  ...
  addCoupon(formSearch as any).then((res: any) => {
    if (res.code == 200) {
      router.go(-1);
    } else {
    }
...
const onOverMax = limit => {
  ...
  // formSearch.discount = formSearch.discount.replace(/[^(\d||.)]/g, '');
  formSearch.afterDiscountPrice = formSearch.price - Math.abs(formSearch.discount);
